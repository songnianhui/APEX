"""Tests for internal staged CLI dispatch helpers."""

import unittest
from types import SimpleNamespace
from unittest.mock import patch

from apex_cas.main import create_parser, _run_buildcas, _run_compute, _run_fcidump, _run_scf


class TestStagedCLICommands(unittest.TestCase):
    ...
    def test_internal_run_scf_uses_scf_only_workflow(self):
        ...
        with patch("apex_cas.main._resolve_case_dir", return_value="demo_case"), patch(
            ...
            "apex_cas.state_io._save_scf_summary"
        ) as mocked_save:
            _run_scf(args)
    ...
    def test_internal_run_buildcas_uses_saved_scf_workflow(self):
        args = SimpleNamespace(
            structure="demo.xyz",
            case_dir="demo_case",
            cas_settings=None,
    ...
    def test_internal_run_compute_uses_split_scf_then_buildcas_flow(self):
        args = SimpleNamespace(
            structure="demo.xyz",
            case_dir="demo_case",
            cas_settings=None,
    ...
    def test_internal_run_fcidump_writes_stage_summary(self):
        args = SimpleNamespace(
            case_dir="demo_case",
            spin_projection=0.5,
            output=None,
        ...
        with patch(
            ...
            "apex_cas.main._find_chkfile",
            return_value="/tmp/demo.chk",
        ), patch(
            "apex_cas.main._resolve_fcidump_selection_path",
            return_value="/tmp/demo_selection.txt",
        ), patch(
            "apex_cas.main._load_active_selection",
            return_value=([1, 2, 3], 5),
        ), patch(
            "apex_cas.FCIDUMP_generator._generate_fcidump_from_selection",
            return_value="/tmp/FCIDUMP.demo",
        ) as mocked_gen, patch(
            "apex_cas.state_io._save_fcidump_summary"
        ) as mocked_save, patch(
        ...
        mocked_save.assert_called_once()
        gen_kwargs = mocked_gen.call_args.kwargs
        self.assertEqual(gen_kwargs["n_electrons"], 5)
        self.assertIsNone(gen_kwargs["frozen_core_indices"])
        save_kwargs = mocked_save.call_args.kwargs
        self.assertEqual(
            save_kwargs["settings_payload"]["active_file"],
            "/tmp/demo_selection.txt",
        )
        self.assertTrue(
            save_kwargs["settings_payload"]["output"].endswith("/outputs/fcidump/FCIDUMP.demo")
        )
